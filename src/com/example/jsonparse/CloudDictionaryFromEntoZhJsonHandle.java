 ...
 * data:[] 这样的是jsonarra
 */
public class CloudDictionaryFromEntoZhJsonHandle implements CloudDictionaryJsonHandle{
	private String dictionary_url="http://api.fanyi.baidu.com/api/trans/vip/translate?from=en&to=z"; // 更新的百度翻译的地址
	private String keys;
	private ArrayList<HashMap<String, String>> dictionary_result;
	private boolean ifreturnresult=true;
	
	public CloudDictionaryFromEntoZhJsonHandle(String keys) {
		...
		try {
			...
			if(jsonObject.getString("errno").equals("0")){
				...
				for (int i = 0; i < symbols.length(); i++) {
					...
					hashMap.put("ph_am", "美"+"【"+symbolitem.getString("ph_am")+"】");
					hashMap.put("ph_en", "英"+"【"+symbolitem.getString("ph_en")+"】");
					JSONArray parts = symbolitem.getJSONArray("parts");
					String explain="";
					for (int j = 0; j < parts.length(); j++) {
						JSONObject part = parts.getJSONObject(j);
						explain += part.getString("part")+"  ";
	...
	protected String getDictionaryJsonContent(){
		...
		try {
			...
			HttpGet httpGet = new HttpGet(dictionary_url+keys+"&from=en&to=zh");
